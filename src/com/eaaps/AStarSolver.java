...
public abstract class AStarSolver {
    public static class Pair {
        ...
        public Pair(int node, float cost) {
            this.node = node;
            this.cost = cost;
        }
    }

    public abstract float heuristic(GraphWithWeights graph, int node, int goal);

    public void solve(GraphWithWeights graph, int start, int goal) {
        if (start == goal) {
        ...
        float[] costSoFar = new float[graph.verticesNum()];
        cameFrom[start] = -1;
        costSoFar[start] = -1;
        ...
        while(!frontier.isEmpty()) {
            ...
            for (int neighbor: graph.neighbors(current.node)) {
                ...
                if (costSoFar[neighbor] == 0 || new_cost < costSoFar[neighbor]) {
                    costSoFar[neighbor] = new_cost + heuristic(graph, current.node, neighbor);
